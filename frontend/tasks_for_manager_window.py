...
def press_task_change_button(user_id, task_change_id_entry, task_change_status_combobox, update_table_method):
    ...
    exit_code = update_task_manager_status(user_id, task_id, status_id)
    if exit_code == 1:
...
def open_tasks_for_manager_window(root, user_id):
    ...
    task_change_status_label = tk.Label(tasks_window, text="Cтатус со стороны координатора:", font=label_font)
    ...
    def update_table():
        ...
        nonlocal user_id
        data = get_manager_tasks(user_id)
